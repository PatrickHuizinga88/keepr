  ...
  <div class="container">
    ...
    <ul>
      <li v-for="memory in collection.memories" :key="memory.id">
        <NuxtLinkLocale
          :to="localePath({name: 'collectionId-memories-slug', params: { collectionId: collection.id, slug: memory.slug }})">
          {{ memory.title }}
        </NuxtLinkLocale>
      </li>
    </ul>

    <NuxtLinkLocale :to="localePath({name: 'collectionId-memories-create', params: { collectionId: collection.id }})"
                    class="border p-4">
      +
    </NuxtLinkLocale>
  </div>
