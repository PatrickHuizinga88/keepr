...
<template>
  <div @drop.prevent="dropHandler" @dragover.prevent="dragOverHandler" @dragleave.prevent="dragLeaveHandler"
    ...
    <input type="file" :multiple="props.multiple" @change="onFileInput" class="absolute inset-0 opacity-0 cursor-pointer" />
...
<!--    </div>-->
  ...
  <div v-if="files.length" class="flex flex-wrap gap-2 w-full !mt-4">
    <div v-for="file in files" :key="file.name" class="relative h-16 w-auto aspect-square">
      ...
      <button @click="removeFile(file)" class="absolute top-1 right-1 bg-foreground/50 rounded-full p-1">
