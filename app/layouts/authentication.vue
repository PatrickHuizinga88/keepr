...
<template>
  <div class="min-h-screen flex flex-col justify-center sm:bg-muted/50 py-12">
    <div
      class="sm:mx-auto sm:w-full sm:max-w-sm text-center px-6 sm:px-0 mb-4 sm:mb-6 md:mb-8 -mt-20 sm:mt-0"
    >
      <img
        src="/logo.svg"
        :alt="appname"
      ...
      />
      <div
        class="text-center mt-10 starting:opacity-0 starting:translate-y-2 transition-all duration-500 delay-100"
      >
...
@media (min-width: 640px) {
  .gradient-shadows {
    box-shadow:
      32px 64px 128px -48px hsl(var(--primary) / 0.1),
      -64px -24px 128px -48px
        color-mix(in lch increasing hue, hsl(var(--primary) / 0.1) 33%, hsl(var(--accent) / 0.1)),
      64px -48px 128px -48px
        color-mix(in lch increasing hue, hsl(var(--primary) / 0.1) 67%, hsl(var(--accent) / 0.1));
  }
}
</style>
