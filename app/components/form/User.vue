...
const roleOptions = [
  ...
  {
    value: "editor",
    label: t("users.roles.editor"),
    description: t("users.roles.editor_description"),
  },
  {
    value: "viewer",
    label: t("users.roles.viewer"),
    description: t("users.roles.viewer_description"),
  },
];
...
<template>
  <form @submit="onSubmit" class="@container grid gap-y-6">
    ...
    <Button :loading="loading" class="w-full lg:w-auto justify-self-end">
      {{
        capitalizeSentence(
          $t(`common.actions.${props.invite ? "invite" : "save"}_item`, {
            item: $t("users.users"),
          }),
        )
      }}
