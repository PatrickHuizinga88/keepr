...
const formSchemaEmail = toTypedSchema(
  z.object({
...
const formSchemaPassword = toTypedSchema(
  z.object({
  ...
  <div class="grid sm:grid-cols-2 gap-6">
    <div>
      ...
      <Form
        ...
        as=""
        keep-values
        :initial-values="initialValuesEmail"
        ...
        <Dialog v-model:open="emailDialogOpen">
          <DialogTrigger as-child>
            <Button variant="outline" size="sm" class="w-full mt-4">
              {{
                capitalizeSentence(
                  $t("common.actions.edit_item", { item: $t("common.general.email") }),
                )
              }}
            </Button>
          </DialogTrigger>
            ...
            <form id="emailForm" @submit="handleSubmit($event, onEmailSubmit)" class="grid gap-4">
              ...
              <DialogFooter>
                <Button type="submit" :loading="loading" class="w-full" form="emailForm">
                  {{
                    capitalizeSentence(
                      $t("common.actions.save_item", { item: $t("common.general.email") }),
                    )
                  }}
                </Button>
              </DialogFooter>
        ...
        <Dialog v-model:open="passwordDialogOpen">
          <DialogTrigger as-child>
            <Button variant="outline" size="sm" class="w-full mt-4">
              {{
                capitalizeSentence(
                  $t("common.actions.edit_item", { item: $t("authentication.common.password") }),
                )
              }}
            </Button>
          </DialogTrigger>
          <DialogContent>
            ...
            <p v-if="passwordChangeMessage" class="text-sm text-destructive">
            ...
            <DialogFooter>
              <Button type="submit" class="w-full" form="passwordForm" :loadng="loadingPassword">
                {{
                  capitalizeSentence(
                    $t("common.actions.save_item", { item: $t("authentication.common.password") }),
                  )
                }}
              </Button>
            </DialogFooter>
