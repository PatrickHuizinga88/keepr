...
const { data: coverImage } = await useAsyncData("coverImage", async () => {
  try {
    if (!memory.value.cover_image) return null;
...
<template>
  <div
    class="relative h-[200px] flex flex-col items-start justify-between rounded-b-2xl py-4 overflow-hidden"
  >
    <img
      v-if="coverImage"
      :src="coverImage"
    ...
    <div class="relative container max-w-[900px]">
      <NuxtLinkLocale :to="`/${route.params.collectionId}`">
