...
<template>
  <Authentication title="">
    ...
    <h1 class="text-3xl font-semibold text-center mb-2">
      {{ $t("authentication.register.account_activated") }}
    </h1>
    <p class="text-muted-foreground text-center">
      {{ $t("authentication.register.welcome_to", { appName: APP_NAME }) }}
    </p>
    <Button class="w-full mt-6" as-child>
      <NuxtLinkLocale to="intro">
        {{ $t("authentication.register.continue") }}
      </NuxtLinkLocale>
    </Button>
  </Authentication>
</template>
